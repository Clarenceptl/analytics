...
import type { ApiDTO, LoginVM, RegisterDTO, UpdateUserDTO } from '@/models';

const API_URL = import.meta.env.VITE_API_URL;
const authToken = localStorage.getItem(TOKEN.BEARER) ?? '';

export class AuthService {
  static async registerUser(user: RegisterDTO): Promise<ApiDTO | any> {
    try {
  ...
  static async updateUser(user: UpdateUserDTO): Promise<ApiDTO | any> {
    try {
      const response = await fetch(`${API_URL}/auth/register`, {
        method: 'POST',
        headers: {
